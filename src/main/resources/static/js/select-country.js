$.widget("jqx.country", {
	...
	_create: function() {
		function build(option) {
			var s = ''
			s += '<i class="flagstrap-icon flagstrap-' + $(option).val().toLowerCase() + '"></i>';
			s += '<span class="country-name">' + $(option).html() + '</span>';
			return s;
		}
		var e = $(this.element);
		for (var c in this.countries) {
			e.append('<option value="' + c + '"' + ($(this.element).attr('data-value')==c ? ' selected="selected" ': '') + '>'
					+ this.countries[c] + '</option>');
		}
